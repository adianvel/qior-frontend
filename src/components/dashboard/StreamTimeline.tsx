...
type StreamTimelineProps = {
  stream: Pick<StreamAccount, "startTime" | "cliffTime" | "endTime" | "milestoneReached" | "milestoneTime">;
  mode: StreamMode;
};
      ...
      <div className="rounded-[28px] border border-zinc-200 bg-white p-5 shadow-[0_18px_60px_rgba(24,24,27,0.045)]">
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          {[
            ...
            { label: "Gate", value: formatDate(stream.milestoneTime) },
            ...
            <div key={item.label} className="relative rounded-2xl border border-zinc-100 bg-zinc-50 p-3.5">
              {index < 3 ? <div className="absolute left-full top-1/2 hidden h-px w-3 -translate-y-1/2 bg-zinc-200 md:block" /> : null}
    ...
    return (
      <div className="rounded-[28px] border border-zinc-200 bg-white p-5 shadow-[0_18px_60px_rgba(24,24,27,0.045)]">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            { label: "Locked", value: "No early unlock" },
            { label: "Unlock", value: formatDate(stream.endTime) },
          ].map((item, index) => (
            <div key={item.label} className="relative rounded-2xl border border-zinc-100 bg-zinc-50 p-3.5">
              {index < 1 ? <div className="absolute left-full top-1/2 hidden h-px w-3 -translate-y-1/2 bg-zinc-200 md:block" /> : null}
              <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-zinc-400">{item.label}</p>
              <p className="mt-1.5 text-sm font-semibold text-zinc-900">{item.value}</p>
            </div>
          ))}
